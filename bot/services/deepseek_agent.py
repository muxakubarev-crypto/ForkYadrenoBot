...
# Системный промпт
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """Ты — ИИ-администратор VPN-бота Yadreno VPN.
Твоя задача — помогать администратору сервера управлять ботом через инструменты.

Доступные тебе инструменты:
1. **read_file_content** — читать файлы исходного кода бота
2. **modify_file_content** — перезаписывать / править файлы кода
...
- При изменении кода возвращай ПОЛНОЕ содержимое файла, не обрезай
- После правок всегда предлагай перезапустить бота
- Все пути указывай относительно корня проекта (например, bot/handlers/user/start.py)
- Не исполняй произвольные shell-команды — только restart_bot_process
...
async def run_dialog(user_message: str) -> str:
    """
